...
function clearElectionTimer(electionId) {
...
export async function getActiveElection(guildId) {
...
export async function registerCandidate(electionId, userId, manifesto) {
    const election = await prisma.election.findUnique({ where: { id: electionId } });
    if (!election || election.status !== "registration")
        throw new ElectionError("not_in_registration");
...
export async function tallyVotes(electionId) {
...
async function announce(client, channelId, description, title) {
    try {
...
async function transitionToVoting(client, electionId) {
    const election = await prisma.election.findUnique({ where: { id: electionId } });
    if (!election || election.status !== "registration")
        return;
    const candidates = await getCandidates(electionId);
...
async function endElection(client, electionId) {
    const election = await prisma.election.findUnique({ where: { id: electionId } });
    if (!election || election.status !== "voting")
        return;
...
/** 任期満了時にロールを剥奪する。 */
async function expireTerm(client, electionId) {
    const election = await prisma.election.findUnique({ where: { id: electionId } });
    if (!election || !election.winnerUserId)
        return;
    try {
        const guild = await client.guilds.fetch(election.guildId);
...
function scheduleTimer(client, key, delay, callback) {
    clearElectionTimer(key);
    const timer = safeSetTimeout(callback, Math.max(0, delay));
    activeTimers.set(key, timer);
}
/** 選挙作成直後にフェーズ移行タイマーを仕込む。 */
export function scheduleElectionPhaseTimers(client, election) {
...
export async function resumeElectionTimers(client) {
...
export async function cancelElection(electionId) {
