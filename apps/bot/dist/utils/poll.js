...
export function buildPollContainer({ question, options, closed, voteCounts, pollId, }) {
    ...
    if (!closed) {
        ...
        for (let i = 0; i < options.length; i += 5) {
            ...
            const row = new ActionRowBuilder().addComponents(chunk.map((opt, j) => new ButtonBuilder().setLabel(`${i + j + 1}`).setStyle(ButtonStyle.Secondary).setCustomId(`poll:vote:${pollId}:${i + j}`)));
            container.addActionRowComponents(row);
        }
    }
    return container;
}
async function getVoteCounts(pollId, optionCount) {
...
export async function refreshPollMessage(client, pollId) {
...
export async function closePoll(client, pollId) {
...
export function schedulePollClose(client, pollId, closesAt) {
