...
export async function applyForCitizenship(guildId, userId, requestedRoleId, reason) {
...
export async function approveCitizenship(guildId, userId, approvedById) {
...
/** 申請を却下する。 */
export async function rejectCitizenship(guildId, userId, rejectedById, reason) {
    const application = await prisma.citizen.findUnique({ where: { guildId_userId: { guildId, userId } } });
    if (!application || application.status !== "pending") {
        throw new CitizenError("no_pending_application");
    }
...
export async function getCitizen(guildId, userId) {
    return prisma.citizen.findUnique({ where: { guildId_userId: { guildId, userId } } });
}
export async function getPendingApplications(guildId, limit = 20) {
