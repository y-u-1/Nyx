 ...
 */
export function scaleToKanji(scale) {
    const table = {
        "-1": "不明",
        10: "震度1",
        20: "震度2",
...
export function scaleToColor(scale) {
    if (scale >= 60)
        return "#8b0000"; // 震度6強以上
    if (scale >= 50)
        return "#d7263d"; // 震度5強〜6弱
    if (scale >= 40)
        return "#f46036"; // 震度4〜5弱
    if (scale >= 30)
        return "#f6c445"; // 震度3
    if (scale >= 10)
        return "#8fd694"; // 震度1〜2
...
export function scaleToAccentColor(scale) {
    return parseInt(scaleToColor(scale).replace("#", ""), 16);
}
//# sourceMappingURL=earthquake-scale.js.map