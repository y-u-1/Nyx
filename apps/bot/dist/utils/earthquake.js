...
import { AttachmentBuilder, ContainerBuilder, MediaGalleryBuilder, MessageFlags, SeparatorBuilder, SeparatorSpacingSize, TextDisplayBuilder } from "discord.js";
 ...
 */
function buildEarthquakeContainer({ title, hypocenterName, magnitude, depth, maxScale, occurredAt, pointLines, tsunami, }) {
...
async function broadcast(client, container, imageBuffer, minScale, maxScale, requireEew, isEew) {
    ...
    // マップ画像はガイルド共通なので、コンテナへのMediaGallery追加はループの外で一度だけ行う
    ...
    for (const settings of targets) {
        if (!settings.earthquakeChannelId)
            continue;
        try {
...
async function handleQuakeInfo(client, data) {
...
async function handleEewWarning(client, data) {
    const hypocenter = data.earthquake?.hypocenter;
    if (!hypocenter?.latitude || !hypocenter?.longitude)
        return;
    const maxScale = data.earthquake?.maxScale ?? -1;
...
export function startEarthquakeListener(client) {
    connect(client);
}
