...
const messageTimestamps = new Map(); // key: `${guildId}:${userId}`
function isBypassed(message, bypassRoleId) {
...
function detectViolation(message, settings) {
    ...
    if (settings.blockInvites && INVITE_PATTERN.test(message.content))
        return "invite";
    if (settings.blockLinks && LINK_PATTERN.test(message.content))
        return "link";
...
function detectSpam(message, threshold, windowSeconds) {
    if (!message.guildId || threshold <= 0)
        return false;
...
export async function checkAutomod(message) {
    ...
    if (!settings)
        return false;
    if (isBypassed(message, settings.bypassRoleId))
        return false;
...
async function enforceViolation(message, violation, timeoutSeconds) {
...
async function logAutomodAction(message, violation, reason) {
    if (!message.guildId)
        return;
    await sendLog(message.client, message.guildId, "spam", "AutoMod", `**User:** <@${message.author.id}>\n**Channel:** <#${message.channelId}>\n**Rule:** \`${violation}\`\n**Reason:** ${reason}`, "warning");
}
