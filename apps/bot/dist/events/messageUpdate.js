...
export function registerMessageUpdateEvent(client) {
    client.on("messageUpdate", async (oldMessage, newMessage) => {
        try {
            if (!newMessage.guildId || newMessage.author?.bot)
                return;
            if (oldMessage.content === newMessage.content)
                return; // 埋め込み展開などcontentが変わらない更新は無視
            ...
            await sendLog(client, newMessage.guildId, "message", "Message Edited", `**Author:** ${newMessage.author ? `<@${newMessage.author.id}>` : "Unknown"}\n**Channel:** <#${newMessage.channelId}>\n\n**Before:**\n${before.slice(0, 500)}\n\n**After:**\n${after.slice(0, 500)}`, "warning");
        }
        catch (error) {
