...
export function registerGuildMemberAddEvent(client) {
    client.on("guildMemberAdd", async (member) => {
        try {
            await handleMemberJoin(member);
            const accountCreatedUnix = Math.floor(member.user.createdTimestamp / 1000);
            await sendLog(client, member.guild.id, "member", "Member Joined", `**User:** <@${member.id}> (\`${member.user.tag}\`)\n**Account created:** <t:${accountCreatedUnix}:R>\n**Member count:** \`${member.guild.memberCount}\``, "success");
