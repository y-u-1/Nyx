...
export const logsConfig = {
    ...
    async execute(interaction) {
        if (!interaction.guildId)
            return;
        const memberLog = interaction.options.getChannel("member_log");
        const messageLog = interaction.options.getChannel("message_log");
        const vcLog = interaction.options.getChannel("vc_log");
        const channelLog = interaction.options.getChannel("channel_log");
        const spamLog = interaction.options.getChannel("spam_log");
        const moderationLog = interaction.options.getChannel("moderation_log");
        const ticketLog = interaction.options.getChannel("ticket_log");
        const redeemLog = interaction.options.getChannel("redeem_log");
