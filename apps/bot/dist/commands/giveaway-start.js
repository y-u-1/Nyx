import { ChannelType, MessageFlags, PermissionFlagsBits, SlashCommandBuilder, TextDisplayBuilder, } from "discord.js";
import { prisma } from "@nyx/database";
import { baseEmbed } from "../utils/embeds.js";
import { parseDuration } from "../utils/duration.js";
import { buildGiveawayContainer, scheduleGiveawayEnd } from "../utils/giveaway.js";
const HEX_COLOR_PATTERN = /^#?[0-9a-fA-F]{6}$/;
export const giveawayStart = {
    data: new SlashCommandBuilder()
        .setName("giveaway-start")
        .setDescription("Start a giveaway in this channel.")
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
        .addStringOption((opt) => opt.setName("prize").setDescription("What are you giving away?").setRequired(true))
        .addStringOption((opt) => opt.setName("duration").setDescription("How long the giveaway runs, e.g. 10m, 2h, 1d").setRequired(true))
        .addIntegerOption((opt) => opt.setName("winners").setDescription("Number of winners (default 1)").setMinValue(1).setMaxValue(20))
        .addChannelOption((opt) => opt.setName("channel").setDescription("Channel to post in (default: this channel)").addChannelTypes(ChannelType.GuildText))
        ...
        .addStringOption((opt) => opt.setName("winners_dm_message").setDescription("Custom DM text for winners. Use {prize} to insert the prize name"))
        ...
        .setMinValue(1)
        .setMaxValue(365))
        .addIntegerOption((opt) => opt.setName("min_level").setDescription("Minimum Nyx. level required to enter").setMinValue(1).setMaxValue(1000)),
    async execute(interaction) {
        ...
        const winnersRole = interaction.options.getRole("winners_role");
        ...
        const contentParts = [pingRole ? `${pingRole}` : null, createMessage].filter(Boolean);
...
//# sourceMappingURL=giveaway-start.js.map