...
import { createUnit, getUnit, listUnits, createRank, listRanks, joinUnit, leaveUnit, getMember, setRank, addPoints, getUnitLeaderboard, MilitaryError, } from "../utils/military.js";
export const military = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((sub) => sub.setName("list").setDescription("List ranks in a unit").addStringOption((opt) => opt.setName("unit").setDescription("Unit name").setRequired(true))))
    ...
    async execute(interaction) {
        ...
        if (sub === "promote") {
            ...
            try {
                ...
                if (member.rank?.roleId)
                    await guildMember.roles.remove(member.rank.roleId).catch(() => { });
                if (rank.roleId)
                    await guildMember.roles.add(rank.roleId).catch(() => { });
            }
            catch (error) {
