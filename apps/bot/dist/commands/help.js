...
function categorize(name) {
    if (name.startsWith("giveaway"))
        return "Giveaway";
    if (name.startsWith("level") || ["rank", "leaderboard", "xp"].includes(name))
        return "Leveling";
    if (["balance", "daily", "pay", "shop", "coin", "inventory"].includes(name))
        return "Economy";
    if (name.startsWith("automod") || name.startsWith("earthquake") || name.startsWith("logs"))
        return "AutoMod & Logs";
    if (["warn", "warnings", "kick", "ban", "unban", "softban", "timeout", "purge", "lock", "unlock", "slowmode", "modlog", "clear-warnings"].includes(name))
        return "Moderation";
    if (name.startsWith("redeem"))
        return "Redeem";
