...
const minuteCounters = new Map<string, number>(); // key: `${guildId}:${userId}`

const TICK_INTERVAL_MS = 60 * 1000;

/** Bot襍ｷ蜍墓凾縺ｫ蜻ｼ縺ｶ縲・蛻・＃縺ｨ縺ｫ蜈ｨ繧ｵ繝ｼ繝舌・縺ｮ繝懊う繧ｹ繝√Ε繝ｳ繝阪Ν繧定ｵｰ譟ｻ縺励※XP繧剃ｻ倅ｸ弱☆繧・*/
export function startVoiceXpTracker(client: Client) {
  setInterval(() => {
...
async function tick(client: Client) {
  for (const guild of client.guilds.cache.values()) {
    ...
    for (const channel of voiceChannels.values()) {
      ...
      if (humanMembers.size < settings.voiceXpMinMembers) continue;
