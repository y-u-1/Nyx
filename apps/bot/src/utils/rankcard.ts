...
function drawProgressBar(ctx: SKRSContext2D, x: number, y: number, w: number, h: number, progress: number) {
...
async function drawAvatarCircle(ctx: SKRSContext2D, avatarUrl: string, cx: number, cy: number, radius: number) {
  try {
...
export async function generateRankCard({ username, avatarUrl, level, currentLevelXp, xpForNextLevel, rank }: RankCardOptions): Promise<Buffer> {
  const width = 800;
  const height = 260;
...
export async function generateLevelUpImage(oldLevel: number, newLevel: number): Promise<Buffer> {
  const width = 420;
  const height = 140;
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext("2d");
