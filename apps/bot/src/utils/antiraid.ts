...
const joinTimestamps = new Map<string, number[]>(); // key: guildId

async function logSecurityEvent(member: GuildMember, description: string) {
...
export async function handleMemberJoin(member: GuildMember) {
  ...
  if (timestamps.length < settings.raidJoinThreshold) return;

  // 繝ｬ繧､繝画､懃衍縲りｨｭ螳壹＆繧後◆繧｢繧ｯ繧ｷ繝ｧ繝ｳ繧貞ｮ溯｡後☆繧九・
  await logSecurityEvent(
    member,
    `Raid detected: \`${timestamps.length}\` joins within ${settings.raidJoinWindowSeconds}s. Action: \`${settings.raidAction}\`.`,
  );
