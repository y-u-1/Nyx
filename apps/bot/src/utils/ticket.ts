...
export function buildTicketChannelPanel({
  ticketNumber,
  openerId,
  claimedById,
  status,
  ...
  if (status === "closed") return { container: panel, row: null };

  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setLabel("Claim").setStyle(ButtonStyle.Primary).setCustomId(`ticket:claim:${ticketNumber}`),
    new ButtonBuilder().setLabel("Close").setStyle(ButtonStyle.Danger).setCustomId(`ticket:close:${ticketNumber}`),
  );
 ...
 */
export async function createTicketChannel(guild: Guild, openerId: string) {
  const settings = await prisma.ticketSettings.findUnique({ where: { guildId: guild.id } });
  if (!settings) return { error: "Ticket system hasn't been set up yet. Ask an admin to run `/ticket`." as const };

  const existing = await prisma.ticket.findFirst({ where: { guildId: guild.id, openerId, status: { not: "closed" } } });
  if (existing) return { error: `You already have an open ticket: <#${existing.channelId}>` as const };
