...
interface EarthquakeMapOptions {
  epicenter: { lat: number; lng: number; name?: string };
  points: MapPoint[];
}

function project(lat: number, lng: number, bounds: typeof JAPAN_BOUNDS, width: number, height: number) {
 ...
 */
export async function generateEarthquakeMap({ epicenter, points }: EarthquakeMapOptions): Promise<Buffer> {
  const width = 800;
  const height = 800;
...
function drawStar(ctx: SKRSContext2D, cx: number, cy: number, outerRadius: number, points: number, color: string) {
