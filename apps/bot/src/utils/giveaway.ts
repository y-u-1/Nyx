...
interface GiveawayPanelData {
  ...
  winnersRoleId?: string | null;
  coinPrize?: number | null;
...
export function buildGiveawayContainer({
  ...
  if (blacklistRoleId) requirementLines.push(`**Blocked role:** <@&${blacklistRoleId}>`);
  if (bypassRoleId) requirementLines.push(`**Bypass role:** <@&${bypassRoleId}>`);
  if (bonusRoleId) requirementLines.push(`**Bonus entries:** <@&${bonusRoleId}> members get \`${bonusEntries}x\` odds`);
  if (minAccountAgeDays) requirementLines.push(`**Minimum account age:** \`${minAccountAgeDays} days\``);
  if (minLevel) requirementLines.push(`**Minimum level:** \`${minLevel}\``);

  const rewardLines: string[] = [];
  if (winnersRoleId) rewardLines.push(`**Winner role:** <@&${winnersRoleId}>`);
  if (coinPrize) rewardLines.push(`**Coin reward:** \`${coinPrize}\` per winner`);
  ...
  if (thumbnailUrl) {
    ...
    if (descriptionDisplay) section.addTextDisplayComponents(descriptionDisplay);
  ...
  if (imageUrl) {
    ...
    container.addMediaGalleryComponents(
      new MediaGalleryBuilder().addItems((item) => item.setURL(imageUrl).setDescription(`Image for ${prize}`)),
    );
  }
  ...
  if (status === "active") {
    ...
    const winnerText =
      winnerIds && winnerIds.length > 0 ? winnerIds.map((id) => `<@${id}>`).join(", ") : "No valid entries.";
...
function pickWinners(entries: { userId: string; weight: number }[], count: number): string[] {
  const pool: string[] = [];
  for (const entry of entries) {
    for (let i = 0; i < Math.max(1, entry.weight); i++) pool.push(entry.userId);
  }
  ...
  while (remaining.length > 0 && winners.length < count) {
    ...
    for (let i = remaining.length - 1; i >= 0; i--) {
      if (remaining[i] === winnerId) remaining.splice(i, 1);
    }
  }
...
async function applyWinnerRewards(client: Client, giveaway: { guildId: string; winnersRoleId: string | null; coinPrize: number | null }, winnerIds: string[]) {
...
export async function endGiveaway(client: Client, giveawayId: string) {
  ...
  try {
    ...
    if (giveaway.dmWinners && winnerIds.length > 0) {
      const dmText = (giveaway.winnersDmMessage ?? "You won **{prize}** in a giveaway. Check the server for details.").replace(
        "{prize}",
        giveaway.prize,
      );
 ...
 */
export async function cancelGiveaway(client: Client, giveawayId: string) {
  clearGiveawayTimer(giveawayId);
 ...
 */
export async function requeueGiveaways(client: Client) {
