import { PermissionFlagsBits, SlashCommandBuilder, type ChatInputCommandInteraction, type Role, type TextChannel, type User } from "discord.js";
...
import {
  ...
  castVote,
  tallyVotes,
  scheduleElectionPhaseTimers,
  cancelElection,
  ElectionError,
} from "../utils/election.js";

export const election: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === "vote") {
      const candidate = interaction.options.getUser("candidate", true) as User;
      const activeElection = await getActiveElection(interaction.guildId);
